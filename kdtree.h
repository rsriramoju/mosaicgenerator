...
class KDTree
{
  ...
  public:
    ...
    /*changed*/
    //helper recursive function for constructor.
  ...
  private:
     ...
     */
     Point<Dim> quickselect(int l, int r, int k, int dim);
     int partition(int l, int r, int m, int dim);
     typename KDTree<Dim>::KDTreeNode* buildtree(KDTreeNode* subroot, int l, int r, int nextDim);
     
     void nearestRecur(const Point<Dim>& query, Point<Dim> & nearest, KDTreeNode * curr, int dim) const;

     typename KDTree<Dim>::KDTreeNode* _copy(KDTreeNode* subroot, KDTreeNode* rhs);
